  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (fromStage <= 2) {
          const { ok } = await runStage(2, "pal4vst", {
          ...
          send({
            ...
            status: "done",
            progress: 100,
            maskPath: `${sessionId}/stage3_mask.png`,
            resultPath: `${sessionId}/stage3_mask.png`,
          });
        ...
        if (fromStage <= 3) {
          ...
          send({ stage: 3, status: "done", progress: 100, resultPath: `${sessionId}/stage4_result.png` });
