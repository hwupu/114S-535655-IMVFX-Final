  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (fromStage <= 1) {
          const { ok } = await runStage(1, "instructpix2pix", {
        ...
        // Stage 2 — Qwen2.5-VL artifact detection
        if (fromStage <= 2) {
          const { ok, status } = await runStage(2, "qwen25vl", {
          ...
          const detected = r?.artifacts ?? [];
          const boxes = r?.boxes ?? [];
          const hasArtifacts = r?.has_artifacts ?? false;

          sessionArtifacts.set(sessionId, detected);
          send({ stage: 2, status: "done", progress: 100, artifacts: detected, rawText, boxes });
